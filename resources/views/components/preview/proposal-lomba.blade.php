@if ($model::find($id)->hasMedia('proposal_lomba'))
    <div
        class="flex items-center space-x-2 p-4 bg-white shadow-md rounded-lg border dark:bg-gray-800 dark:border-gray-700">

        <a href="{{ $model::find($id)->getFirstMediaUrl('proposal_lomba') }}" target="_blank" rel="noopener noreferrer"
            class="text-blue-600 dark:text-blue-400 hover:underline font-semibold">
            Lihat Proposal Lomba
        </a>
    </div>
@else
    <div
